...
class BaseRegistry(Generic[K, V], ABC):
    ...
    def get(self, key: K) -> V:
        ...
        if key not in self._items:
            ...
            raise KeyError(
                f"'{key}' not found in {self._name}. " f"Available: {available}"
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"{self.__class__.__name__}(name='{self._name}', size={len(self._items)})"
        )
...
class FunctionRegistry(BaseRegistry[str, Callable]):
    ...
    def register_function(
        self, name: str, func: Callable, overwrite: bool = False
    ) -> None:
        """
    ...
    def register_module_functions(
        self, module, prefix: str = "", overwrite: bool = False
    ) -> None:
        """
        Register all functions from a module.
...
class ClassRegistry(BaseRegistry[str, Type[T]]):
    ...
    def __init__(
        self, name: str = "class_registry", base_class: Optional[Type[T]] = None
    ):
        """
...
class ProviderRegistry(ClassRegistry):
    ...
    def __init__(
        self, name: str = "provider_registry", base_class: Optional[Type] = None
    ):
...
def create_class_registry(
    name: str = "classes", base_class: Optional[Type[T]] = None
) -> ClassRegistry[T]:
...
def create_provider_registry(
    name: str = "providers", base_class: Optional[Type] = None
) -> ProviderRegistry:
