    ...
    if dimensions_format == "relative":
        if slides_app is None:
            raise ChartGenerationError(
                "slides_app must be provided for relative dimensions conversion"
            )

        def get_page_dimension(dim: str) -> float:
            dim_info = slides_app["pageSize"][dim]
            return (
                dim_info["magnitude"] / 12700
                if dim_info["unit"] == "EMU"
                else dim_info["magnitude"]
            )
...
def apply_alignment(
    ...
    try:
        ...
        raise ChartGenerationError(
            f"Invalid alignment format: {alignment_format}. Expected format: 'horizontal-vertical'"
        )
    ...
    if horizontal_align == "center":
        ...
        raise ChartGenerationError(
            f"Invalid horizontal alignment: {horizontal_align}. Supported: 'left', 'center', 'right'"
        )
    ...
    if vertical_align == "center":
        ...
        raise ChartGenerationError(
            f"Invalid vertical alignment: {vertical_align}. Supported: 'top', 'center', 'bottom'"
        )
...
def compute_chart_dimensions(
    ...
    if width_pt <= 0 or height_pt <= 0:
        raise ChartGenerationError(
            f"Width and height must be positive, got: {width_pt}x{height_pt}"
        )

    if width_pt > page_width_pt * 2 or height_pt > page_height_pt * 2:
        raise ChartGenerationError(
            f"Chart dimensions too large: {width_pt}x{height_pt} (page: {page_width_pt}x{page_height_pt})"
        )
    ...
    if abs(x_pt) > page_width_pt * 3 or abs(y_pt) > page_height_pt * 3:
        raise ChartGenerationError(
            f"Final position too far from slide: ({x_pt}, {y_pt})"
        )
