...
class PresentationBuilder:
    ...
    def _build_slide(cls, spec) -> Slide:
        ...
        """
        replacements: List[BaseReplacement] = []
        for repl_spec in spec.replacements:
            replacement = cls._build_replacement(repl_spec)
            replacements.append(cast(BaseReplacement, replacement))
        ...
        return Slide(
            id=spec.id, title=spec.title, replacements=replacements, charts=charts
        )
    ...
    def _build_replacement(cls, spec) -> ReplacementUnion:
        ...
        adapter: TypeAdapter[Any] = TypeAdapter(ReplacementUnion)
    ...
    def _build_chart(cls, spec) -> ChartUnion:
        ...
        adapter: TypeAdapter[Any] = TypeAdapter(ChartUnion)
        return adapter.validate_python(config)
    ...
    def _build_data_source(cls, config: Union[Dict[str, Any], List[Dict[str, Any]]]):
        ...
        adapter: TypeAdapter[Any] = TypeAdapter(DataSourceConfig)
        return adapter.validate_python(config)
