...
def create_traffic_light_colors(
    value: Any, good_threshold: float, warning_threshold: float
) -> str:
...
def create_growth_colors(
    df: pd.DataFrame, column_order: List[str], growth_columns: List[str]
) -> pd.DataFrame:
    ...
    """
    return create_dynamic_colors(
        df, column_order, growth_color_function, growth_columns
    )
