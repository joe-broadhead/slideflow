...
class TextReplacement(BaseReplacement):
    ...
    type: Annotated[
        Literal["text"], Field("text", description="Discriminator for text replacement")
    ]
    placeholder: Annotated[
        str, Field(..., description="Placeholder to replace (e.g. '{{STORE_CODE}}')")
    ]
    replacement: Annotated[
        Optional[str],
        Field(
            default=None, description="Static text to use if no `value_fn` is provided"
        ),
    ]
    data_source: Annotated[
        ...
        Field(
            default=None,
            description="Optional data source for computing the replacement",
        ),
    ]
    value_fn: Annotated[
        ...
        Field(
            default=None,
            description="Function to compute the replacement text from data",
        ),
    ]
    value_fn_args: Annotated[
        Dict[str, Any],
        Field(
            default_factory=dict,
            description="Keyword arguments passed to `value_fn`; string values will be formatted",
        ),
    ]
    ...
    def get_replacement(self) -> str:
        ...
        if self.value_fn:
            ...
            if data is not None:
                ...
                return str(
                    self.value_fn(transformed_data, **(self.value_fn_args or {}))
                )
