...
class AITextReplacement(BaseReplacement):
    ...
    type: Annotated[
        Literal["ai_text"],
        Field("ai_text", description="Discriminator for AI-text replacements"),
    ]
    placeholder: Annotated[
        str, Field(..., description="Placeholder to replace (e.g. '{{SUMMARY}}')")
    ]
    prompt: Annotated[
        str, Field(..., description="Prompt template for the AI provider")
    ]
    provider: Annotated[
        Union[str, Type[AIProvider], AIProvider, Callable[..., str]],
        Field(
            default="openai",
            description="Provider name, class, instance, or plain callable",
        ),
    ]
    provider_args: Annotated[
        Dict[str, Any],
        Field(
            default_factory=dict, description="Keyword args for provider init & call"
        ),
    ]
    data_source: Annotated[
        Optional[List[DataSourceConfig]],
        Field(default=None, description="Optional data source(s) to include"),
    ]
    ...
    def _validate_data_source(cls, values: Any) -> Any:
        if (
            ...
            and "data_source" in values
            and values["data_source"] is not None
            and not isinstance(values["data_source"], list)
        ):
            values = dict(values)
    ...
    def _prepare_provider(self) -> tuple[Callable[..., str], dict[str, Any]]:
        ...
        if callable(Prov):
            return cast(Callable[..., str], Prov), args
