...
class TableColumnFormatter(BaseModel):
    ...
    format_fn: Annotated[
        Callable[[Any], str],
        Field(..., description="Function to format a cell's value"),
    ]
    format_fn_args: Annotated[
        Dict[str, Any],
        Field(default_factory=dict, description="Additional kwargs for `format_fn`"),
    ]
...
class TableFormattingOptions(BaseModel):
    ...
    custom_formatters: Annotated[
        Dict[str, TableColumnFormatter],
        Field(default_factory=dict, description="Map column name → formatter"),
    ]
...
class TableReplacement(BaseReplacement):
    ...
    type: Annotated[
        Literal["table"],
        Field("table", description="Discriminator for table replacements"),
    ]
    prefix: Annotated[
        str, Field(..., description="Prefix for table placeholders (e.g. 'REPORT_')")
    ]
    data_source: Annotated[
        Optional[DataSourceConfig],
        Field(default=None, description="Config of the data source"),
    ]
    formatting: Annotated[
        TableFormattingOptions,
        Field(default_factory=TableFormattingOptions, description="Column formatters"),
    ]
    replacements: Annotated[
        Optional[Dict[str, Any]],
        Field(
            default=None, description="Static placeholder→value map (skips data source)"
        ),
    ]
    ...
    def _validate_source_or_replacements(self):
        if self.replacements is None and self.data_source is None:
            raise ValueError(
                "Table replacement requires either `replacements` or `data_source`."
            )
        return self
