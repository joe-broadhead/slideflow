...
def apply_data_transforms(
    data_transforms: Optional[List[Dict[str, Any]]], df: pd.DataFrame
) -> pd.DataFrame:
    ...
    for i, transform in enumerate(data_transforms):
        if "transform_fn" in transform and callable(transform["transform_fn"]):
            ...
            try:
                ...
                logger.debug(
                    f"Applied transform {i+1}/{len(data_transforms)}: {func.__name__}"
                )
                ...
                logger.error(
                    f"DataFrame shape before transform: {transformed_df.shape}"
                )
